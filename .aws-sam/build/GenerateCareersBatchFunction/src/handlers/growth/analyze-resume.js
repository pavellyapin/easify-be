...
exports.lambdaHandler = async (event) => {
    try {
        ...
        const [chatGPTSecret, resumeAnalysisPrompt, serviceAccountKey] = await Promise.all([
            getDataFromS3(process.env.SECRETS_S3_BUCKET_NAME, process.env.SECRETS_S3_KEY_NAME, 'gptSecret'),
            getDataFromS3(process.env.PROMPTS_S3_BUCKET_NAME, process.env.PROMPTS_S3_KEY_NAME, 'resumeAnalysisPrompt'),
        ...
        const resumeAnalysisMessage = { role: 'user', content: `${resumeAnalysisPrompt}\n\n${extractedText['extractedText']}` };
        const conversation = [resumeAnalysisMessage];

        console.log('Sending conversation to GPT-4:', conversation);
        const gptResponse = await getChatGPTPrompt(openai, conversation, 4096, 'gpt-4o-mini');
        ...
        const resumeRef = firestore.collection(`users/${userId}/resumes`).doc(fileName).collection('reports').doc('scan');
        await resumeRef.set({
