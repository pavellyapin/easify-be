...
exports.lambdaHandler = async (event) => {
    try {
        ...
        const [chatGPTSecret, industryRequest, serviceAccountKey] = await Promise.all([
            getDataFromS3(process.env.SECRETS_S3_BUCKET_NAME, process.env.SECRETS_S3_KEY_NAME, 'gptSecret'),
            getDataFromS3(process.env.PROMPTS_S3_BUCKET_NAME, process.env.PROMPTS_S3_KEY_NAME, 'careersRequest'),
        ...
        for (const industryMsg of industries) {
            ...
            const existingIndustry = await checkIfIndustryExists(firestore, industryMsg);
            if (existingIndustry) {
                console.log(`Industry with name "${industryMsg}" already exists. Skipping generation.`);
                continue;
            }

            const industryRequestMessage = { role: 'user', content: `Industry name: ${industryMsg} ${industryRequest}` };
            ...
            const gptResponse = await getChatGPTPrompt(openai, conversationWithIndustryRequest, 4096, 'gpt-4o-mini');
...
const checkIfIndustryExists = async (firestore, industryName) => {
    const industriesCollection = firestore.collection('industries');
    const querySnapshot = await industriesCollection.where('name', '==', industryName).get();
