...
exports.lambdaHandler = async (event) => {
    try {
        ...
        const [chatGPTSecret, financialPlanRequest, serviceAccountKey] = await Promise.all([
            getDataFromS3(process.env.SECRETS_S3_BUCKET_NAME, process.env.SECRETS_S3_KEY_NAME, 'gptSecret'),
            getDataFromS3(process.env.PROMPTS_S3_BUCKET_NAME, process.env.PROMPTS_S3_KEY_NAME, 'financialPlanRequest'),
        ...
        for (const financialPlanMsg of financialPlans) {
            ...
            const existingPlan = await checkIfFinancialPlanExists(firestore, financialPlanMsg.name);
            if (existingPlan) {
                console.log(`Financial plan with name "${financialPlanMsg.name}" already exists. Skipping generation.`);
                continue;
            }
            ...
            const conversationWithFinancialPlanRequest = [financialPlanRequestMessage];

            console.log('Prepared conversation:', conversationWithFinancialPlanRequest);
            const gptResponse = await getChatGPTPrompt(openai, conversationWithFinancialPlanRequest, 8212, 'gpt-4o-mini');
        ...
        return {
            statusCode: 200,
            headers: HEADERS,
            body: JSON.stringify({ message: 'Financial plans processed successfully.' })
        };
    } catch (error) {
...
exports.saveFinancialPlanToFirestore = async (firestore, financialPlan) => {
    ...
    if (typeof financialPlan.tags === 'string') {
        financialPlan.tags = financialPlan.tags.split(',').map(tag => tag.trim().toLowerCase());
    } else if (Array.isArray(financialPlan.tags)) {
        // Normalize the tags array to lowercase
        financialPlan.tags = financialPlan.tags.map(tag => tag.toLowerCase().trim());
    }
    ...
    console.log('Financial plan successfully saved to Firestore with ID:', document.id);
};
...
const checkIfFinancialPlanExists = async (firestore, financialPlanName) => {
    const financialPlansCollection = firestore.collection('financialPlans');
    const querySnapshot = await financialPlansCollection.where('name', '==', financialPlanName).get();
